defmodule Agx.Searcher do
    ...
    base
      ...
      |> Enum.map(fn file ->
        ...
        check(file_path, args_config, global_config, %{
          ...
          ignored_folder: (Enum.member?(global_config.ignored_folders, file) || hidden_dir?(file_path, args_config[:all])),
  ...
  def hidden_dir?(_, true) do
    false
  end

  def hidden_dir?(file, _) do
    File.dir?(file) && String.starts_with?(Path.basename(file), ".")
  end
end
